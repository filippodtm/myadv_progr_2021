...
def test_ctor():     #qualsiasi cosa inizi con test... 
    s = Sequence(range(3))
    assert s.l == [0,1,2]
...
#Fixture: starting point that can be used for different tests (NI)
@pytest.fixture     #to test always on the same thing (defaultseq)
...
def test_iter(default_sequence):
    ...
    # now we expect an exception (quindi avrò errore se non esce exception?)
