...
from typing import Tuple, Sequence, Mapping, Callable, Iterable, NamedTuple
...
def label(
    ...
    d = defaultdict(list)   #d è un diz, per ogni centroide(key) c'è una lista di punti
    for p in dataset:
        pdist = partial(dist, p)      #funzione dist applicata a una parte di argomenti
...
def update_centroids(clusters: Iterable[Cluster]) -> Sequence[Centroid]:
    return [Point(*map(mean,zip(*cluster)) for  #d c
    
    #centroids = []
    #for cluster in clusters:
     #   xc, yc = list(zip(*cluster))
     #  centroids.append(Point(mean(xc), mean(yc)))
    #return centroids
...
def distortion(
    ...
    for centroid, cluster in labeled_dataset.items():
        pdist = partial(distance, centroid)               #SDM
...
if __name__ == "__main__":
    ...
    with open("s3.txt") as f:
        points = [Point(*map(float, line.split())) for line in f]
