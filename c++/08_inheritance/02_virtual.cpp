...
//FUNCTION OVERRIdING: same function, same argument(s)?, different output
...
struct Animal {
  ...
  //mark it virtual if theres at least ONE virtual function
};
//VIRTUAL : permette ai children di usare la funzione che sta nel parent




struct Dog : public Animal {
  void speak() const noexcept override { std::cout << "Bau\n"; } //override (non riscrivere virtual) permette di cambiare output type
  Dog() = default;
...
struct Snake : public Animal {
  ...
  void info() const noexcept override {
    Animal::info();         //occhio a specificare il FULLNAME
