...
//use classes that implement IIS one at a time
//dont use rawpointers, use uniquepointers
 ...
 public:
  ManyResources() : ptr{nullptr}, v{3} {
    ...
    try {
      ...
      v= 2;    //perche sta riga? non viene eseguita
    } catch (...) {
      delete[] ptr;  // <----
      throw;         // re-throw
    }
...
int main() {
  ...
  try {
    // int* raw_ptr=new int[7]; // wrong because raw_ptr would not be visible inside the catch-clause
    ManyResources mr;
    Bar b;
